...
Builder.prototype.build = function(tree) {
  ...
  tree.cssRules.forEach(function(rule) {
    if (rule.media) {
      medias.push(self.media(rule));

      var context = self.mediaContext(rule);
      if (mediaz[context]) { mediaz[context].cssRules = mediaz[context].cssRules.concat(rule.cssRules); }
      else { mediaz[context] = rule; }
  ...
  for (var context in mediaz) {
    if (mediaz.hasOwnProperty(context)) {
      output += "@media " + context + "{" + this.build(mediaz[context]) + "}";
    }
  }
