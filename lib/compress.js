  ...
  tree.rules.forEach(function(rule, i) {
    ...
    if (typeof rule.media !== 'undefined') {
      if (mediaCache[rule.media]) {
        var last = mediaCache[rule.media];

        rule.rules = last.rule.rules.concat(rule.rules);
        delete(tree.rules[last.index]);
      }

      mediaCache[rule.media] = {rule: rule, index: i };
    }
...
Transformer.prototype.consolidateViaDeclarations = function(rule, context, i, cache) {
  return this.consolidate('selectors', 'declarations', rule, context, i, cache);
};
...
Transformer.prototype.consolidateViaSelectors = function(rule, context, i, cache) {
  return this.consolidate('declarations', 'selectors', rule, context, i, cache);
};

Transformer.prototype.consolidate = function(what, via, rule, context, i, cache) {
